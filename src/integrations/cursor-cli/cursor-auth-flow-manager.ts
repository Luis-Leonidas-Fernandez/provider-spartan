...
import type { CursorCliCapabilities, CursorCliStatusSnapshot } from "./cursor-cli.types.js";
import type {
  CursorAuthEvent,
  CursorAuthFlowManagerPort,
  CursorAuthFlowSnapshot,
  CursorInteractiveProcess,
  CursorInteractiveProcessLauncher,
} from "./cursor-auth.types.js";

type InternalFlow = {
  snapshot: CursorAuthFlowSnapshot;
  emitter: EventEmitter;
  process: CursorInteractiveProcess;
...
function isExpired(snapshot: CursorAuthFlowSnapshot, now = new Date()) {
  return snapshot.status === "running" && new Date(snapshot.expiresAt).getTime() <= now.getTime();
}

function withUpdatedSnapshot(flow: InternalFlow, event: CursorAuthEvent) {
  flow.snapshot = {
    ...flow.snapshot,
    updatedAt: nowIso(),
...
function classifyInputRequirement(text: string): CursorAuthEvent | null {
  const normalized = text.toLowerCase();
  if (/enter .*code|paste .*code|verification code|authorization code|device code|one[- ]time code/.test(normalized)) {
...
function resolveLoginArgs(capabilities: CursorCliCapabilities | null) {
  if (capabilities?.supportsNoBrowserLogin) return ["login", "--no-browser"];
  return ["login"];
}

export class CursorAuthFlowManager implements CursorAuthFlowManagerPort {
  private readonly flows = new Map<string, InternalFlow>();

  constructor(
    private readonly launcher: CursorInteractiveProcessLauncher,
    private readonly statusInspector: { inspect(): Promise<CursorCliStatusSnapshot> },
    private readonly capabilitiesInspector: { inspect(): Promise<CursorCliCapabilities | null> },
  ...
  async start(): Promise<CursorAuthFlowSnapshot> {
    ...
    internal.expiryTimer = setTimeout(() => {
      ...
      withUpdatedSnapshot(internal, { type: "failed", code: "AUTH_FLOW_EXPIRED", message: "Cursor auth flow expired" });
    ...
    const handleChunk = (raw: string, stream: "stdout" | "stderr") => {
      const text = redactCursorCliOutput(raw);
      if (!text.trim()) return;
    ...
    process.onError((error) => {
      if (internal.snapshot.status !== "running") return;
      internal.snapshot.status = "failed";
      withUpdatedSnapshot(internal, { type: "failed", code: "PROCESS_ERROR", message: redactCursorCliOutput(error.message) });
    });
    process.onExit(async () => {
      ...
      withUpdatedSnapshot(internal, {
        ...
        message: status.message ?? "Cursor CLI auth flow ended without ready status",
      });
    });
  ...
  async writeInput(flowId: string, input: string): Promise<CursorAuthFlowSnapshot> {
    this.cleanupExpired();
  ...
  async cancel(flowId: string): Promise<CursorAuthFlowSnapshot> {
    const flow = this.flows.get(flowId);
    if (!flow) throw new Error(`Auth flow ${flowId} not found`);
  ...
  subscribe(flowId: string, listener: (event: CursorAuthEvent) => void) {
    this.cleanupExpired();
    const flow = this.flows.get(flowId);
    if (!flow) return null;
    const wrapped = (event: CursorAuthEvent) => listener(event);
  ...
  cleanupExpired(now = new Date()) {
    ...
    for (const flow of this.flows.values()) {
      ...
      withUpdatedSnapshot(flow, { type: "failed", code: "AUTH_FLOW_EXPIRED", message: "Cursor auth flow expired" });
      flow.process.kill("SIGTERM");
      cleaned += 1;
    }
    return cleaned;
  ...
  cancelAll(reason = "Cursor auth manager shutdown") {
    let cancelled = 0;
    for (const flow of this.flows.values()) {
