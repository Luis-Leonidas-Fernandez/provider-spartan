...
function auditFilename(event: CodexRequestAuditEvent) {
  const timestamp = event.occurredAt.replace(/[:.]/g, "-");
  return `${timestamp}_${safeSegment(event.providerId)}_${event.phase}.json`;
}

export class FileCodexRequestAuditRecorder implements CodexRequestAuditRecorderPort {
  constructor(private readonly directory: string) {}

  async record(event: CodexRequestAuditEvent) {
